...
import com.mobile.cristiannieto.androidissuereporter.aws.Util;
import com.mobile.cristiannieto.androidissuereporter.aws.Constants;
...
public class UploadActivity extends ListActivity {
    ...
    @TargetApi(23)
    private void PictureAccessPermision(){
    ...
    private void initData() {
        ...
        for (TransferObserver observer : observers) {
            ...
            if (TransferState.WAITING.equals(observer.getState())
                ...
                Log.e("AWS", observer.getAbsoluteFilePath());
            }
        }
    ...
    private void updateList() {
        ...
        for (int i = 0; i < observers.size(); i++) {
            ...
            if (TransferState.COMPLETED.equals(observer.getState())) {
                String S3Url = "![image](https://s3-us-west-2.amazonaws.com/androidissuereportertest/" + FilenameUtils.getName(observer.getAbsoluteFilePath()) + ")";
                ClipboardManager clipboard = (ClipboardManager) getSystemService(CLIPBOARD_SERVICE);
                ClipData clip = ClipData.newPlainText("AWS", S3Url);
                clipboard.setPrimaryClip(clip);
                Toast.makeText(this, "Added image uploaded to AWS S3 to Clipboard!", Toast.LENGTH_SHORT).show();
                try {
                    Thread.sleep(1000);
                } catch (Exception e) {
                    Log.e("Thread Error", "There was a exception during the thread sleep");
                }
                Log.e("AWS", S3Url);
                Log.e("AWS", S3Url);
                this.finish();
